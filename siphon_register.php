<?php
$p_title = "Registeration-Siphon | ChES";
...
if (isset($_SESSION['id'])){
	...
	if(isset($_GET['deluser'])){
		...
		$query2 = mysqli_query($dbc,"SELECT * FROM siphon WHERE id = $ide");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
	...
	$query3 = mysqli_query($dbc,"SELECT * FROM siphon_register WHERE t_id = '$tid'");
	$row3 = mysqli_num_rows($query3);
	
	if($row3 == 0){

			// Add the user to the database:
			$q = "INSERT INTO siphon_register (u_id, e_id, qty, amt, t_id, phone, date) VALUES ('$uid', '$eid', '$qty', '$amt', '$tid', '$ph', NOW())";
